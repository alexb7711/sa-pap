...
properties. These properties are enforced by a set of constraints derived from the MILP PAP TODO: \cite{myself}. 
The constraints must ensure no overlap temporally or spacially, receives enough charge to complete route after each 
visit $i$, bus visit $i$ cannot be overcharged, each visit, $i$, departs on time. The aforemention constraints are shown
in [[eq:constraints]].
...
\begin{equation}
\begin{array}{ll}
d_j - u_i < \simga_i^1                                                   & \text{Valid queue position/time} \\
u_j - d_i < \sigma_i^2                                                   & \text{Valid queue position/time} \\
v_i - v_j = \sigma_i^3                                                   & \text{Valid queue position/time} \\
\sigma_i^1 + \sigma_i^2 + \sigma_i^3 \geq 0                              & \text{Valid queue position/time} \\
...
d_i \leq e_i                                                             & \text{Detach time should be less than or equal to departure} \\
s_i = d_i - u_i                                                          & \text{Time spent on charger is equal to the difference of the attach and detach time} \\
\end{array}
\end{equation}
#+END_EXPORT

Where the valid queue position/time constraint is as defined in \cite{tutorials_point} and depicted in [[fig:valid-queue]].
Also note that the $\eta$ constraints can only be verified /after/ the schedule has been generated as the initial charge
for each visit is based from the previous charger selection and charge time.
