...
bus per mile and $\rho_{\Isetfinal}$ is the route mileage after visit $i$. $\delta_i$ is calculated using $\delta_i =
\text{average mph} \cdot (a_{\xi_i - d_i})$. The average MPH is assumed to be 20 miles per hour.
...
demand at any discrete time, consider [[eq:discrete-power]]. Let $\omega_h$ be the discrete power demand at time step $h$
where $h \in \{ 1, 2, ..., H \} \subset \mathcal{Z}$ and $H = \frac{T}{900}$. For conciseness of notation we will abuse
$t_h$ to denote the time in discrete form (as opposed to $t$ being continuous), let $dt_h = t_h - t_{h-1}$, and
$\Hset = \{ 1, 2, ..., H \}$. Let $\iota_h$ be a binary decision variable that is enabled if charger $v_i \in \Qset$ is
enabled in the time frame $[t_{h-1}, t_h]$, where $t_h = 900 \cdot h$. Let the power usage of charger $v_i$ be denoted
as $r_{v_i}$.
...
\label{eq:discrete-power}
  \omega = \sum_h^H \sum_i^I \iota_h \cdot r_{v_i}
\end{equation}
#+end_export

The average power can be rewritten as $p_{15} = \omega$, $p_{dem}$ can be rewritten as $p_{dem} = \text{max}_{h \in
\Hset} (p_{15}^{h})$, and finally $p_d$ can be written as [[eq:pd-dis]].

#+name: eq:pd-dis
#+begin_export latex
\begin{equation}
\label{eq:pd-dis}
  p_d = \text{max}_{h \in \Hset}(p_{fix}, p_{max})s_d
\end{equation}
#+end_export
...
\begin{equation}
\label{eq:pc}
\PC = p_d + \sum_{i=1}^I \Big( r[v_i](d_i - u_i) \Big)
\end{equation}
#+end_export
