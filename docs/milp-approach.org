...
little more formally, [[eq:bus-spat-temp]] describes the relationship that $\sigma_{ij}$ and $\phi_{ij} uphold. That is, for
every visit, if the start charge time of either visit $i$ or $j$ is greater than end charge time of the other, then
$\omega_{ij}$ is active. Similarly, if the queue for visit $i$ and $j$ are different then $\psi_{ij}$ is active. These
...
The final set of constraints \ref{seq:c11} - \ref{seq:c15} set the rules for the decision variable $\iota_h$.
...
where $\alpha = 0.995$ in [[fig:cool]]. An Exponential cooling schedule is defined by the difference equation is define as
[[eq:cool2]].
...
and lines 9 and 10 add it to the set of candidates. Line 14 chooses picks a candidate solution, $x_i' \subset X_i'$,
...
[[alg:find-free-time]] is a range for which the bus may be charged and the empty set if it cannet. As an example, consider a
