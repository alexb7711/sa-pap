...
:END:
Slide visit is used for buses that have already been scheduled. Because of the constraint \ref{seq:c10}
there may be some room to move $u_i$ and $d_i$ within the window $[a_i, e_i]$. Two new values, $u_i$ and
$d_i$ are selected with a uniform distribution to satisfy the constraint $a_i \leq u_i \leq d_i \leq e_i$.
Lines 1 and 2 get the arrival and departure times, and line 3 gets the current queue that visit $i$ is
assigned to.
