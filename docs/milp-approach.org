...
* Problem Description
Given a set of bus arrivals to a charging station $i= \{1,...,I\} \in \mathbb{Z} with a set of chargers to be queued $q = \{1,...,Q\} \in \mathbb{Z} where the bus is idetified by a identificaiton number $b = \{1,...,B\}$ \in \mathbb{Z}$.
