...
generator. The algorithm is outlined in [[alg:perturb-schedule]].
...
related to any of the variables presented for SA PAP thus far. Consider [[alg:sa-pseudo]] \cite{Henderson}. $\omega$
and $\omega'$ are the current solution and the candidate solution, respectively. $t_k$ is the temperature cooling schedule,
$T$ is the initial temperature that will iterate until $k = K$. $M_k$ is the repetition counter, it defines the number
of iterations to execute for each temperture $t_k$.
