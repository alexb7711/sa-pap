...
impl TempFunc {
    ...
    pub fn new(schedule: CoolSchedule, init_temp: f32, delta: f32, autorun: bool) -> TempFunc {
        ...
        let d: f32 = match schedule {
            ...
            // Geometric should be less than 1
            {
                if delta.abs() > TempFunc::EXP_MAX {
                    TempFunc::EXP_MAX
                } else {
                    delta.abs()
                }
    ...
    fn autorun(self: &mut TempFunc) {
        while let Some(_) = self.exec_cool_schedule() {}
    }
